...
export const Video = ({ data }: { data: PageBlocksVideo }) => {
  ...
  if (!data.url || (data.tag && activeFilter !== data.tag)) {
    return null;
  }
  return (
    <Section background={data.background!} className={`aspect-video ${data.color}`}>
      <ReactPlayer width='100%' height='100%' style={{ margin: 'auto' }} playing={!!data.autoPlay} loop={!!data.loop} controls={!!data.controls} url={data.url} muted={!!data.muted} preload="true" />
    </Section>
  );
};
